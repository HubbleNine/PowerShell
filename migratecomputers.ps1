<#
Author: Me/Collaborative
Run, select OU to move computers from, select OU to move computers to, magic
#>

#Function for choosing OU, windows form
...
$TargetOU = (Choose-ADOrganizationalUnit).DistinguishedName
$SourceOU = (Choose-ADOrganizationalUnit).DistinguishedName
$SourceComputers = Get-ADComputer -Filter * -SearchBase $SourceOU -SearchScope OneLevel
$countPC = ($SourceComputers).count
Write-Host " Starting import computers ..."
  
Write-Host " Moving Computer Accounts..." 
$SourceComputers | Move-ADObject -TargetPath $TargetOU
