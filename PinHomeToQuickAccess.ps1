###Name: PinHomeToQuickAccess.ps1
###Author(s): Me, function to actually pin to Quick Access written by Johan Carlsson
###Date: 07/23/2019
###Purpose: To pin a users home directory, in my case, a user share on the network, 
############to the Quick Access section in Windows Explorer

#Set function for Set-QuickAccess, author information in function notes
...
#Set variable from home directory, then parse off the colon at the end to be useable with Get-PSDrive
$myhome=$env:HOMEDRIVE.ToString()
$myhome=$myhome -replace ".$"

#Set variable for the path to map to Quick Access
$mypath=(Get-PSDrive $myhome).DisplayRoot


#Call the function Set-QuickAccess with the flags to pin with the path in variable set before
Set-QuickAccess -Action Pin -Path $mypath
